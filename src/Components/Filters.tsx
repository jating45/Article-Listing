...
const FilterPage: React.FC<FilterProps> = ({ onFilterChange }) => {
    ...
    const fetchAuthors = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data.authors)) {
          const authorNames = response.data.authors.map((item: { author: string }) => item.author);
          setAuthors(["All", ...authorNames]);
        } else {
          console.error("Unexpected Authors API Response:", response.data);
        }
      } catch (err) {
  ...
  // 🔥 **Trigger Filtering Whenever a Selection Changes**
  useEffect(() => {
    onFilterChange({
  ...
  }, [selectedCategory, selectedAuthor, dateSortOrder, onFilterChange]);

  if (loading) return <p className="text-center text-primary">Loading filters...</p>;
